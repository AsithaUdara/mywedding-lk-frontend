// File: src/features/event-planning/components/InviteMemberModal.tsx
"use client";

import React, { useState } from 'react';
import { useAuth } from '@/context/AuthContext';
import { inviteOrganizer } from '@/lib/api/events';
...
const InviteMemberModal = ({ isOpen, onClose, eventId, onInviteSuccess }: InviteMemberModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // We send 'Editor' for 'Parent' role as a business rule, otherwise 'Viewer'
      const determinedPermissionLevel = role === 'Parent' ? 'Editor' : permissionLevel;

      await inviteOrganizer(token, eventId, {
        email,
        role,
        permissionLevel: determinedPermissionLevel,
      });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="relative w-full max-w-lg p-8 rounded-xl shadow-2xl bg-cream" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label htmlFor="inviteRole" className="block text-sm font-medium text-charcoal mb-2">Assign Role</label>
            <div className="relative">
                <select
                    id="inviteRole"
                    value={role}
