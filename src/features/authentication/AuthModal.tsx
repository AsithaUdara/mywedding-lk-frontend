    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="relative w-full max-w-md p-8 rounded-xl shadow-2xl" style={{ backgroundColor: 'var(--color-cream)' }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          {/* FIXED: Added a container div for robust alignment */}
        ...
        <form className="space-y-6">
          {view === 'signUp' && (<div className="relative"><UserIcon size={20} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"/><input type="text" placeholder="Full Name" className="w-full py-3 pl-12 pr-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-accent outline-none"/></div>)}
          <div className="relative"><Mail size={20} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"/><input type="email" placeholder="Email Address" className="w-full py-3 pl-12 pr-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-accent outline-none"/></div>
          <div className="relative"><Lock size={20} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"/><input type="password" placeholder="Password" className="w-full py-3 pl-12 pr-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-accent outline-none"/></div>

          {/* UPDATED: Changed from elegant-lift-button to smooth-scale-button */}
          <button type="submit" className="w-full py-3 rounded-lg text-white font-semibold shadow-lg smooth-scale-button" style={{ backgroundColor: 'var(--color-primary)' }}>
        ...
        <p className="text-center mt-8">
          ...
          {/* FIXED: Removed hover:underline */}
          <button onClick={() => setView(view === 'signIn' ? 'signUp' : 'signIn')} className="font-bold opacity-80 hover:opacity-100 transition-opacity" style={{ color: 'var(--color-primary)' }}>
