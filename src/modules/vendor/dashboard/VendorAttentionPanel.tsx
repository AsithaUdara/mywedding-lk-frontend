...
import type { VendorAttentionItem } from "@/modules/vendor/dashboard/vendorDashboardHelpers";
import { GlassSectionCard } from "@/modules/vendor/dashboard/glass-ui";
import { vg } from "@/modules/vendor/dashboard/vendor-glass-theme";
import { cn } from "@/shared/lib/cn";

function AttentionRow({ item }: { item: VendorAttentionItem }) {
  return (
    <li>
      <Link
        href={item.href}
...
export function VendorAttentionPanel({ items }: { items: VendorAttentionItem[] }) {
  return (
    <GlassSectionCard
      title="Needs attention"
      subtitle="Booking requests, messages, and storefront tasks"
    >
      {items.length === 0 ? (
        <div
          className={cn(
        ...
        >
          ...
          <div>
            ...
            <p className="mt-0.5 text-sm">
              No pending bookings or unread inquiries. Check business analytics for revenue and pipeline.
            </p>
          </div>
        </div>
      ) : (
