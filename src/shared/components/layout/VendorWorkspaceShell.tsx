...
export type VendorNavItem = {
  ...
  badge?: number;
  /** When set, only exact path matches (e.g. dashboard root) */
  exact?: boolean;
};
...
export function VendorWorkspaceShell({
  ...
  const renderNavLink = (item: VendorNavItem, showLabel: boolean, onNavigate?: () => void) => {
  ...
  const sidebarContent = (showLabels: boolean, onNavigate?: () => void) => (
    <>
      <div className="relative border-b border-workspace-sidebar-border bg-gradient-to-b from-primary/[0.06] to-transparent px-4 py-4">
        <Link
          href="/vendor/dashboard"
          className="flex min-w-0 items-center gap-3"
          onClick={onNavigate}
        >
          <div className="relative flex h-11 w-11 flex-shrink-0 items-center justify-center overflow-hidden rounded-xl border border-workspace-sidebar-border bg-card p-1.5 shadow-sm">
      ...
      {mobileNavOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <aside className="absolute left-0 top-0 flex h-full w-[min(100%,18rem)] flex-col border-r border-workspace-sidebar-border bg-workspace-sidebar shadow-xl">
            <div className="flex items-center justify-end border-b border-workspace-sidebar-border p-2">
              <button
                type="button"
                onClick={() => setMobileNavOpen(false)}
        ...
        <header className="sticky top-0 z-30 border-b border-border bg-card/95 backdrop-blur-md">
          <div className="flex h-14 items-center justify-between gap-3 px-4 md:h-16 md:px-8">
            <div className="flex min-w-0 flex-1 items-center gap-2 md:gap-4">
              ...
              >
                <Link href="/vendor/dashboard" className="font-medium hover:text-primary">
                  Vendor
                </Link>
              ...
              <p className="truncate text-sm font-semibold text-foreground sm:hidden">
                {activeItem?.label ?? "Vendor"}
              </p>
            </div>

            <div className="hidden max-w-md flex-1 md:block lg:max-w-lg">
              <label className="sr-only" htmlFor="vendor-workspace-search">
                Search workspace
              </label>
              <div className="relative">
                ...
                />
                <input
                  id="vendor-workspace-search"
                  type="search"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder="Search inquiries, services, bookings…"
